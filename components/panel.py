...
class Panel:
    def __init__(self, windows):
        ...
        # Movements viewer
        self.frame_movements_viewer = None
        self.movements_viewer_canvas = None
        self.movements_viewer_framescroll = None
    ...
    def create_movements_viewer_widgets(self):
        ...
        self.frame_movements_viewer.configure(bg=pv.GRAY, relief="solid", bd=1)
        ...
        title_movements_viewer.configure(bg=pv.GRAY, fg="white")
        ...
        title_n_movement.configure(bg=pv.GRAY, anchor="w")
        ...
        title_white_movements.configure(bg=pv.GRAY, anchor="w")
        ...
        title_black_movements.configure(bg=pv.GRAY, anchor="w")
        ...
        self.movements_viewer_canvas.configure(bg=pv.GRAY, highlightthickness=0)
        self.movements_viewer_canvas.grid(row=2, column=0, columnspan=3, sticky="nsew", pady=(5, 5))

        self.movements_viewer_framescroll = tk.Frame(self.movements_viewer_canvas, bg=pv.GRAY)
    ...
    def insert_movement(self, pieces, movement):
        ...
        if pieces == "whites":
            ...
            row_label = tk.Label(self.movements_viewer_framescroll, width=3, bg=pv.GRAY)
            ...
            white_label = tk.Label(self.movements_viewer_framescroll, width=7, bg=pv.GRAY,
            ...
            back_label = tk.Label(self.movements_viewer_framescroll, bg=pv.GRAY,
                             fg="white", anchor="w", text=movement, font=("Calibri Bold", 12))
    ...
    def create_movements_controller_widgets(self):
        ...
        self.previous_fen_bttn.configure(bg=pv.GRAY, activebackground=pv.GRAY, image=self.img_back)
        self.previous_fen_bttn.grid(row=0, column=0)

        self.first_fen_bttn = tk.Button(ctlr_frame, width=69, height=40, bd=0)
        self.first_fen_bttn.configure(bg=pv.GRAY, activebackground=pv.GRAY, image=self.img_first_fen)
        self.first_fen_bttn.grid(row=0, column=1)

        self.last_fen_bttn = tk.Button(ctlr_frame, width=69, height=40, bd=0)
        self.last_fen_bttn.configure(bg=pv.GRAY, activebackground=pv.GRAY, image=self.img_last_fen)
        self.last_fen_bttn.grid(row=0, column=2)

        self.next_fen_bttn = tk.Button(ctlr_frame, width=69, height=40, bd=0)
        self.next_fen_bttn.configure(bg=pv.GRAY, activebackground=pv.GRAY, image=self.img_next)
        self.next_fen_bttn.grid(row=0, column=3)

    # Chat
    def create_chat_widgets(self):
        self.chat_frame = tk.Frame(self.windows, bg=pv.DARK_GRAY, relief="solid", bd=1)
        self.chat_frame.grid(row=2, column=0)

        self.chat_text = tk.Text(self.chat_frame, width=38, height=10, bg=pv.GRAY)
        ...
        entry_frame = tk.Frame(self.chat_frame, height=30, bg=pv.GRAY)
        ...
        self.chat_entry = tk.Entry(entry_frame, width=35, bg=pv.GRAY, relief="flat", fg="white")
        self.chat_entry.pack(padx=2, pady=5, fill="both", expand=True)

        boton_enviar = tk.Button(self.chat_frame, width=1, bg=pv.GRAY, relief="solid", bd=1)
        boton_enviar.configure(text=">", font=("Arial", 7), fg="white")
        boton_enviar.configure(activebackground=pv.GRAY, activeforeground="white")
    ...
    def send_message(self):
        ...
        if pv.partide:
            pv.partide.send_data(f"message<>{message}")

        if message:
            self.receive_message(message)

    def receive_message(self, message, message_by="main_player"):
        self.chat_entry.delete(0, tk.END)
        self.chat_text.configure(state='normal')
        if message_by == "main_player":
